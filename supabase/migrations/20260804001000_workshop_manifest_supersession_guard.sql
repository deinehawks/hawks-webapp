-- Phase 3I-B follow-up: allow approved manifests to be superseded without
-- allowing broad edits to approved records.
--
-- The previous migration makes approved manifests immutable. This replacement
-- trigger function preserves that behavior while allowing one narrow transition:
-- an approved manifest may be marked superseded when a replacement manifest is
-- recorded in superseded_by_manifest_id. Other approved-row content remains
-- immutable.
