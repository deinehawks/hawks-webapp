      ...
      <Card className="max-w-3xl rounded-lg">
        ...
        <CardContent>
          <form action={createOutput} className="grid gap-4">
            ...
            <label className="grid gap-2 text-sm font-medium">Output type<select className="h-9 rounded-md border bg-background px-3 text-sm" defaultValue="" name="outputType" required><option disabled value="">Select output type</option>{SURVEY_OUTPUT_TYPE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
