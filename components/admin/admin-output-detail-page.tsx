...
import { setCurrentOutput, transitionOutputStatus, updateOutput } from "@/lib/actions/admin-outputs";
...
export default async function AdminOutputDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="@container/main flex flex-1 flex-col gap-6 p-4 md:p-6">
      ...
      <section className="grid gap-3"><div><h2 className="font-semibold">Storage references</h2><p className="text-sm text-muted-foreground">Display-only values managed by the approved asset pipeline.</p></div><div className="grid gap-3 rounded-lg border bg-card p-4 text-sm"><div><span className="font-medium">Bucket:</span> <span className="break-all text-muted-foreground">{output.storage_bucket ?? "Not set"}</span></div><div><span className="font-medium">Path:</span> <span className="break-all text-muted-foreground">{output.storage_path ?? "Not set"}</span></div></div></section>
