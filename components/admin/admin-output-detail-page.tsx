...
export default async function AdminOutputDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const currentEligible = ["ready", "approved"].includes(output.status);
  const readinessItems = [
    { complete: hasStorage, detail: hasStorage ? "Bucket and prefix are attached." : "Attach the existing bucket and prefix before marking ready.", label: "Storage reference" },
    { complete: currentEligible, detail: currentEligible ? "Status can be selected as current." : "Move the output to ready or approved.", label: "Ready status" },
    { complete: output.is_current, detail: output.is_current ? "This is the selected output for its survey/type." : "Set as current after it is ready or approved.", label: "Current selection" },
  ];
  ...
  return (
    <main className="@container/main flex flex-1 flex-col gap-6 p-4 md:p-6">
      <div className="flex flex-col gap-4">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">Manage catalog metadata and readiness. Storage prefixes and publication remain controlled by separate workshop workflows.</p>
      </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="rounded-lg"><CardHeader><CardTitle className="text-sm font-medium">Storage readiness</CardTitle></CardHeader><CardContent><Badge variant={hasStorage ? "default" : "secondary"}>{hasStorage ? "Prefix attached" : "Prefix missing"}</Badge></CardContent></Card>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <Card className="rounded-lg"><CardHeader><CardTitle className="text-base">Readiness status</CardTitle></CardHeader><CardContent className="grid gap-4"><p className="text-sm text-muted-foreground">Ready and approved states require both a bucket and storage prefix or object key. Published status is intentionally unavailable here.</p><div className="flex flex-wrap gap-2">{availableTransitions.map((status) => <TransitionButton key={status} outputId={output.id} status={status} />)}{availableTransitions.length === 0 ? <span className="text-sm text-muted-foreground">No status actions available.</span> : null}</div></CardContent></Card>
        <Card className="rounded-lg"><CardHeader><CardTitle className="text-base">Current output</CardTitle></CardHeader><CardContent className="grid gap-4"><p className="text-sm text-muted-foreground">The current record is the preferred {formatLabel(output.output_type)} for this survey. Selection replaces the previous current record atomically.</p>{currentEligible && !output.is_current ? <form action={setCurrentOutput}><input name="outputId" type="hidden" value={output.id} /><Button type="submit">Set as current</Button></form> : <Badge className="w-fit" variant={output.is_current ? "default" : "secondary"}>{output.is_current ? "Selected" : "Not eligible"}</Badge>}</CardContent></Card>
      </section>

      <section className="grid gap-3"><div><h2 className="font-semibold">Storage prefix/key</h2><p className="text-sm text-muted-foreground">Attach an existing object key or package prefix. This workflow records metadata only; it does not upload, move, publish, or delete files.</p></div><Card className="rounded-lg"><CardContent className="pt-6">{locked ? <div className="grid gap-3 text-sm"><div><span className="font-medium">Bucket:</span> <span className="break-all text-muted-foreground">{output.storage_bucket ?? "Not set"}</span></div><div><span className="font-medium">Prefix/key:</span> <span className="break-all text-muted-foreground">{output.storage_path ?? "Not set"}</span></div></div> : <form action={updateOutputStorageReference} className="grid gap-4 lg:grid-cols-2"><input name="outputId" type="hidden" value={output.id} /><label className="grid gap-2 text-sm font-medium">Bucket<input className="h-9 rounded-md border bg-background px-3 text-sm" defaultValue={output.storage_bucket ?? ""} maxLength={100} name="storageBucket" pattern="[a-z0-9][a-z0-9._-]*[a-z0-9]" placeholder="survey-outputs" required /></label><label className="grid gap-2 text-sm font-medium">Prefix or object key<input className="h-9 rounded-md border bg-background px-3 text-sm" defaultValue={output.storage_path ?? ""} maxLength={1024} name="storagePath" placeholder="tiles/barbco2026/2026/AH-0260001" required /></label><div className="flex items-center justify-between gap-3 lg:col-span-2"><p className="text-sm text-muted-foreground">A bucket and prefix/key are required before moving an output to ready or approved.</p><Button type="submit">Save storage reference</Button></div></form>}</CardContent></Card></section>
