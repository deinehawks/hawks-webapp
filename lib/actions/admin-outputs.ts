...
type OutputRow = Pick<Tables<"survey_outputs">, "id" | "survey_id" | "output_type" | "status" | "is_current" | "storage_bucket" | "storage_path">;

type OutputInsertTable = {
  insert(values: TablesInsert<"survey_outputs">): {
    select(columns: "id"): {
      single(): PromiseLike<{ data: Pick<Tables<"survey_outputs">, "id"> | null; error: PostgrestError | null }>;
    };
  };
};
...
function readRequiredString(formData: FormData, key: string): string {
  ...
  if (typeof value !== "string" || value.trim().length === 0) throw new Error("Missing required output field.");
  return value.trim();
}
...
async function assertPlatformAdmin() {
  ...
  if (profile.role !== "platform_admin") throw new Error("Only platform admins can manage survey outputs.");
  return profile;
}
...
async function loadOutput(outputId: string): Promise<OutputRow> {
  ...
  const { data, error } = await supabase
    .from("survey_outputs")
    .select("id, survey_id, output_type, status, is_current, storage_bucket, storage_path")
    .eq("id", outputId)
    .maybeSingle();
  if (error) throw new Error("Failed to verify survey output.", { cause: error });
...
export async function createOutput(formData: FormData) {
  ...
  const supabase = await createClient();
  const outputsTable = supabase.from("survey_outputs") as unknown as OutputInsertTable;
  const { data, error } = await outputsTable.insert({
    survey_id: surveyId,
  ...
  if (error) throw new Error("Failed to create survey output.", { cause: error });
...
export async function updateOutput(formData: FormData) {
  ...
  const surveyId = readRequiredString(formData, "surveyId");
  const outputType = parseOutputType(readRequiredString(formData, "outputType"));
  await assertSurveyExists(surveyId);
  if (output.is_current && (surveyId !== output.survey_id || outputType !== output.output_type)) {
  ...
  if (error) throw new Error("Failed to update survey output.", { cause: error });
...
export async function transitionOutputStatus(formData: FormData) {
  ...
  if (!allowedTransitions[output.status].includes(nextStatus)) {
    throw new Error(`Cannot move an output from ${output.status} to ${nextStatus}.`);
  }
  if (["ready", "approved"].includes(nextStatus) && (!output.storage_bucket?.trim() || !output.storage_path?.trim())) {
    throw new Error("Storage bucket and path are required before this output can become ready or approved.");
  }

  const supabase = await createClient();
  const outputsTable = supabase.from("survey_outputs") as unknown as OutputUpdateTable;
  const { error } = await outputsTable.update({ status: nextStatus, updated_at: new Date().toISOString() }).eq("id", outputId);
  if (error) throw new Error("Failed to update output status.", { cause: error });
  revalidateOutputPaths(outputId, output.survey_id);
}
...
export async function setCurrentOutput(formData: FormData) {
  ...
  const outputId = readRequiredString(formData, "outputId");
  const output = await loadOutput(outputId);
  if (!["ready", "approved"].includes(output.status)) throw new Error("Only ready or approved outputs can be current.");

  const supabase = (await createClient()) as unknown as OutputRpcClient;
  const { error } = await supabase.rpc("admin_set_current_survey_output", { target_output_id: outputId });
  if (error) throw new Error("Failed to select current output.", { cause: error });
  revalidateOutputPaths(outputId, output.survey_id);
}
