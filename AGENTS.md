...
- The additive domain foundation now includes `organization_types`, `people`, `organizations`, client mapping tables, memberships, farms, survey/farm and survey/organization junctions, explicit grants, `survey_outputs`, and `admin_audit_log`. Phase 3F enables only platform-admin legacy-client classification-field updates on `clients`; Phase 3G-A packages audit triggers for `client_people` and `client_organizations`; Phase 3G-B maps legacy clients to existing canonical people or organizations through checked RPCs; Phase 3G-C creates minimal canonical people/organizations and immediately maps them to a legacy client; Phase 3H-A exposes read-only membership readiness; Phase 3H-B creates ordinary member memberships for existing profiles and existing organizations; Phase 3H-C manages ordinary membership status without deleting records or changing roles. Org-admin promotion, Auth-user creation, farm, grant, output, asset, and destructive mutations remain blocked until controlled workflows, local/staging validation, and rollback paths are reviewed.
