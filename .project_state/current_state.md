...
Protected asset implementation planning is approved in `docs/protected-asset-implementation-plan.md`. This repo remains strictly Next.js app-side; NGINX/Compose config belongs to the existing external WSL Docker infrastructure. Initial app-side implementation is in progress: `lib/assets/protected-asset-auth.ts`, `lib/assets/minio-aliases.ts`, `app/internal/asset-auth/route.ts`, a middleware no-redirect exception for `/internal/asset-auth`, and `public.authorize_workshop_protected_asset(...)` backed by `app_private.lookup_protected_asset_manifest_entry(...)`.
